...
import { Ticket, Wallet, TrendingUp, AlertCircle, Clock } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {/* We would fetch actual stats here, mocking for layout */}
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {user?.operationModel === "prepaid" ? "Token Balance" : "Today's Stakes"}
            </CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {user?.operationModel === "prepaid" ? "Available for betting" : "Total bets placed today"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Active tickets for today's games
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Recent Winnings</CardTitle>
            <TrendingUp className="h-4 w-4 text-green-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-600">GHS --.--</div>
            <p className="text-xs text-muted-foreground mt-1">
              From last 7 days
            </p>
          </CardContent>
